...
> **Front-end to Phase 0.** The user states a goal in natural language; this phase converts it into the precise objective function `f` the framework requires, then hands it to Phase 0 → A → … → solve. The conversion is automatable — an LLM drafts `f` well — but this is the **most dangerous step in the pipeline**: it is where the *formalizability wall* and the *specification-vs-intent gap* live. A flawless framework run on the wrong `f` produces a rigorous, correct, useless answer.

**Input:** the goal in natural language.  **Output:** a validated, precisely-posed `f` + labeled inputs + an achievability tag, handed to Phase 0.

- **F1 — Extract intent.** Identify the decision variable(s) `x`, what "success" means, the direction (max/min), the hard constraints, and the time horizon.
- **F2 — Draft candidate objective function(s).** Write `f(x)` explicitly. If the goal admits several plausible formalizations, ENUMERATE them — never silently pick one. The chosen `f` determines the answer; this is "asking the right question" made explicit and checkable.
- **F3 — Label every input.** Tag each input DESCRIPTIVE / STRUCTURAL / PREDICTIVE (same rule as Phase 0). Flag any missing predictive input now, before effort is spent. An unvalidated predictive assumption smuggled into `f` becomes an OD-1-type defect at the objective level.
- **F4 — Formalizability gate.** If the goal cannot be reduced to a *measurable* `f` (subjective, ill-posed, no measurable success criterion), STOP and return "not formalizable / underspecified — here is what's missing." Do NOT fabricate an `f`. You cannot solve what you cannot state.
- **F5 — Intent verification (spec-vs-intent).** Restate `f` in plain language, with edge cases, and confirm it captures what the user meant. The framework verifies against `f`, not against intent — this confirmation is mandatory, not optional.
- **F6 — Achievability pre-tag.** Classify the goal engine-limited vs reality-limited and note which of the three walls (empirical / formalizability / complexity) it touches, so the user learns up front whether the solver can even in principle deliver.

**Then hand the validated `f` to Phase 0.** If F4 or F6 returns "impossible / not formalizable," report that honestly and stop — that is a valid, valuable outcome, not a failure.
