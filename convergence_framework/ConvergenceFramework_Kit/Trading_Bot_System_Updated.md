...
> **Front-end to the framework.** Before this system's `f(x)` (§3.1) exists, a goal must be turned into it. Phase F converts a natural-language goal into the precise objective function, then hands it to Phase 0 → A → … → solve. The conversion is automatable — an LLM drafts `f` well — but it is the **most dangerous step**: it is where the *formalizability wall* and the *specification-vs-intent gap* live. A flawless run on the wrong `f` produces a rigorous, correct, useless answer.

- **F1 — Extract intent.** Decision variable(s) `x`, meaning of "success", direction, hard constraints, time horizon.
- **F2 — Draft candidate `f`(s).** Write `f(x)` explicitly; enumerate plausible formalizations rather than silently picking one.
- **F3 — Label every input.** DESCRIPTIVE / STRUCTURAL / PREDICTIVE. A descriptive input smuggled in as predictive becomes an OD-1-type defect at the objective level.
- **F4 — Formalizability gate.** If the goal cannot be reduced to a measurable `f`, STOP and report "not formalizable / underspecified." Do not fabricate an `f`.
- **F5 — Intent verification.** Restate `f` in plain language with edge cases; confirm it matches intent. Mandatory.
- **F6 — Achievability pre-tag.** Engine-limited vs reality-limited; which of the three walls it touches.

> *Applied to this system:* the `f(x)` in §3.1 is the output of Phase F. OD-1 is an F3 failure surfaced late — a descriptive score composite treated as a predictive signal.
