...
> **Front-end to Phase 0.** The user states a goal in natural language; this phase converts it into the precise objective the framework requires, then hands it to Phase 0 → A → … → converge. The conversion is automatable — an LLM drafts the objective well — but this is the **most dangerous step in the pipeline**: it is where the *formalizability wall* and the *specification-vs-intent gap* live. A flawless review run against the wrong objective produces a rigorous, correct, useless answer.
...
- **F3 — Label every input.** Tag each input DESCRIPTIVE / STRUCTURAL / PREDICTIVE (same rule as Phase 0). Flag any missing predictive input now, before effort is spent. An unvalidated predictive assumption smuggled into the objective becomes an OD-1-type defect at the objective level.
...
- **F6 — Achievability pre-tag.** Classify the goal engine-limited vs reality-limited and note which of the three walls (empirical / formalizability / complexity) it touches, so the user learns up front whether solve mode can even in principle deliver.
