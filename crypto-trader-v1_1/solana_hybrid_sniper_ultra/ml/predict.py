...
class Predictor:
    def __init__(self):
        ...
        self.model_path = os.path.join(script_dir, "ml", "model.pkl")
        self.model = None
    ...
    def predict(self, features):
        # Always compute heuristic fallback for comparison
        ...
        try:
            ...
            h_prob = heuristic["pump_probability"]

            # If ML model gives very low prob (<0.15) on data where heuristic
            # says >0.45, the model is seeing out-of-distribution data.
            # Fall back to heuristic directly (not a blend — the ML is untrustworthy).
            if raw_prob < 0.15 and h_prob > 0.45:
                logger.debug(f"Heuristic override: ML={raw_prob:.3f} adj={adjusted_prob:.3f} Heuristic={h_prob:.3f}")
                heuristic["model_version"] = "heuristic_override"
                heuristic["penalties"].append(f"ml_uncalibrated(raw={raw_prob:.3f})")
                return heuristic

            # Moderate disagreement: average them
            if adjusted_prob < 0.5:
                adjusted_prob = (adjusted_prob + h_prob) / 2
                penalties.append(f"heuristic_avg(ml={raw_prob:.3f},h={h_prob:.3f})")

            adjusted_prob = max(0.02, min(0.95, adjusted_prob))
            ...
            return {
                ...
                "penalties": penalties,
                "model_version": f"hybrid_v{self.version}",
            }
        except Exception as e:
