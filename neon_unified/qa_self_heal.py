...
class GenAgent:
    ...
    def _call(self, user: str, system: Optional[str] = None, max_tokens: int = 8192) -> str:
        ...
        # Reuse the same resilient call pattern the main agent uses.
        if hasattr(self.pool, "next_available"):
            for attempt in range(6):
                ...
                for _ in range(20):
                    ...
                    time.sleep(1.5)
                if not provider:
                    raise RuntimeError(f"[{self.role}] no provider available")
                try:
                    ...
                    if attempt == 5:
                        raise RuntimeError(f"[{self.role}] NIM exhausted: {e}") from e
                    time.sleep(2.0 + attempt)
        raise RuntimeError(f"[{self.role}] no usable pool")
